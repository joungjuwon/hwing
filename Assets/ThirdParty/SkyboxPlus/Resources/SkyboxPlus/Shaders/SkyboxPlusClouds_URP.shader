Shader "Skybox/Skybox Plus URP/Clouds" {
Properties {
...
SubShader {
    ...
    Pass {
        ...
        TEXTURECUBE(_Tex4); SAMPLER(sampler_Tex4);
        ...
        // Custom uniforms for sun/moon positioning
        float4x4 _LightWL;
        ...
        half3 BlendClouds(half3 color, TEXTURECUBE_PARAM(cube, smp), float3 uv, half alpha, half3 skyColor) {
            half4 tex = SAMPLE_TEXTURECUBE(cube, smp, uv);
            tex.rgb *= skyColor;
            tex.a *= alpha;
        ...
        Varyings vert(Attributes input)
        {
            ...
            o.ray = input.positionOS.xyz;
            
            #if SKYBOX_LAYER_1
                o.uvLayer1 = RotateAroundYInDegrees(input.positionOS.xyz, _Time.x * _RotationSpeed1 + _HShiftLayer1);
                o.uvLayer1.y += sin(_Time.x) * _PhaseLayer1 - _VShiftLayer1;
            #else
                o.uvLayer1 = 0;
            #endif
            
            #if SKYBOX_LAYER_2
                o.uvLayer2 = RotateAroundYInDegrees(input.positionOS.xyz, _Time.x * _RotationSpeed2 + _HShiftLayer2);
                o.uvLayer2.y += sin(_Time.x) * _PhaseLayer2 - _VShiftLayer2;
            #else
                o.uvLayer2 = 0;
            #endif
            
            #if SKYBOX_LAYER_3
                o.uvLayer3 = RotateAroundYInDegrees(input.positionOS.xyz, _Time.x * _RotationSpeed3 + _HShiftLayer3);
                o.uvLayer3.y += sin(_Time.x) * _PhaseLayer3 - _VShiftLayer3;
            #else
                o.uvLayer3 = 0;
            #endif
            
            #if SKYBOX_LAYER_4
                o.uvLayer4 = RotateAroundYInDegrees(input.positionOS.xyz, _Time.x * _RotationSpeed4 + _HShiftLayer4);
                o.uvLayer4.y += sin(_Time.x) * _PhaseLayer4 - _VShiftLayer4;
            #else
                o.uvLayer4 = 0;
            #endif
                ...
                o.hazeRay = normalize(mul(rotHaze, input.positionOS.xyz));
            #else
                o.hazeRay = 0;
            #endif

            #if SBP_CUSTOM_POSITIONS
                o.scrPos = mul(_LightWL, float4(input.positionOS.xyz, 0));
                o.scrPosMoon = mul(_MoonLightWL, float4(input.positionOS.xyz * _WorldSpaceMoonDir.w, 0));
            #else
                o.scrPos = o.positionCS;
                // Get main light direction in URP
                Light mainLight = GetMainLight();
                o.lightPos = TransformObjectToHClip(mainLight.direction);
            #endif
        ...
        half4 frag(Varyings i) : SV_Target
        {
            ...
            half3 sunColorFinal = LinearColor(_SunColor);
            #if SBP_CUSTOM_POSITIONS
                sunColorFinal *= (i.scrPos.z <= 0);
            #endif
            half3 sunFlareColor = sunColorFinal * sunFlare;
            sunColorFinal *= sunIntensity;
            half3 moonColor = 0.0.xxx;
                ...
                if (dist < 0.3) {
                    ...
                    half3 moonTex = SAMPLE_TEXTURE2D_LOD(_MoonTex, sampler_MoonTex, float2(scrDistMoon.xy * _MoonSize + 0.5), 0).rgb;
                    moonColor += moonTex;
                    #if SBP_CUSTOM_POSITIONS
                        moonColor = max(0.0.xxx, moonColor - sunColorFinal);
                    #else
                        sunColorFinal = 0;
                    #endif
                }
            ...
            // final color
            half3 col = skyColor + sunColorFinal + sunFlareColor * 0.5 + (moonColor + starColor) * saturate(ray.y + 0.2);
            ...
            // blend clouds
            #if SKYBOX_LAYER_1
                col = BlendClouds(col, TEXTURECUBE_ARGS(_Tex1, sampler_Tex1), i.uvLayer1, _AlphaLayer1, skyTint);
            #endif
            #if SKYBOX_LAYER_2
                col = BlendClouds(col, TEXTURECUBE_ARGS(_Tex2, sampler_Tex2), i.uvLayer2, _AlphaLayer2, skyTint);
            #endif
            #if SKYBOX_LAYER_3
                col = BlendClouds(col, TEXTURECUBE_ARGS(_Tex3, sampler_Tex3), i.uvLayer3, _AlphaLayer3, skyTint);
            #endif
            #if SKYBOX_LAYER_4
                col = BlendClouds(col, TEXTURECUBE_ARGS(_Tex4, sampler_Tex4), i.uvLayer4, _AlphaLayer4, skyTint);
            #endif
