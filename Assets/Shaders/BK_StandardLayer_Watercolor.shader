...
Shader "Custom/BK_StandardLayered_Watercolor"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
            ...
            sampler2D _WatercolorNoise;
            
            float3 SampleWatercolorNoise(float2 uv)
            {
                return tex2D(_WatercolorNoise, uv).rgb;
            }
						 ...
						 ) : SV_Target
			{
				...
				surfaceData.clearCoatSmoothness = 1;
				...
				#else
                    // --- WATERCOLOR LOGIC START ---
                    // 1. Distortion
                    float2 noiseUV = inputData.positionWS.xz * _DistortionScale;
                    float3 noise = SampleWatercolorNoise(noiseUV);
                    float3 distortion = (noise - 0.5) * _DistortionStrength;
                    float3 distortedPosWS = inputData.positionWS + distortion;
                    
                    // Recalculate Shadow Coord
                    inputData.shadowCoord = TransformWorldToShadowCoord(distortedPosWS);
                    
                    // 2. Lighting Calculation (Inline Watercolor Model)
                    Light mainLight = GetMainLight(inputData.shadowCoord);
                    float3 lightDir = normalize(mainLight.direction);
                    float NdotL = dot(inputData.normalWS, lightDir);
                    ...
                    half4 color = half4(finalColor, surfaceData.alpha);
                    // --- WATERCOLOR LOGIC END ---
				#endif
